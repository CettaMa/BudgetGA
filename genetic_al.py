...
def inisialisasi_populasi(populasi):
    for x in range(jumlah_k):
        ...
        kro = [] #
        for _ in range(7):
            kro.append(numpy.random.randint(0,budget))
        for _ in kro:
...
def crossover(populasi,pc):
    ...
    for x in pasangan:
        rules_kawin = numpy.random.uniform(0,1)
        if rules_kawin < pc: # kalo rules nya lebih kecil daripada propability Crossover, pasanngan bakal di kawinin, yeyyy kawinnnn
            print(f"Kawin Antara {x[0][0]} dan {x[0][1]} !! dengan rules : {rules_kawin}\n")
            ...
            if sum(offspring2)>budget:
                delta_offsprint = abs(sum(offspring2)-budget)
                offspring2 = [x - (delta_offsprint/len(offspring2)) for x in offspring2]
            # else : 
            #     delta_offsprint = abs(sum(offspring1)-budget)
            #     # offspring1 = [x + (delta_offsprint/len(offspring1)) for x in offspring1]
...
# buat berat dari setiap kromosom [Casing,Prosesor,Ram,Storage,Cooling,PSU,kartu Grafis]
berat_gaming = [10,70,50,30,40,15,80]

#inisialisasi populasi
if __name__ == "__main__":
    budget = float(input("Masukan Budget Anda :   Juta \x1B[8D"))
    jumlah_k =int(input("Masukan Jumlah kromosom :"))
    ...
    while ittera < generation:
        ...
        print(f"\nProses Crossover 🔄 :")
        crossover(population,prob_cross)
        pol_fitness=kalkulasi_fitness(population,berat_gaming)
        mutasi(population,prob_mutation)
        elitism(population,k_fitness)
        pol_fitness = kalkulasi_fitness(population,berat_gaming)
        if ittera == 0:
            initial_fitness = pol_fitness
        ittera += 1
        mean_fitness.append((nested_sum(pol_fitness)/len(pol_fitness)))
        max_fitness.append(max(pol_fitness))
        min_fitness.append(min(pol_fitness))
    ...
    plotting.plot(range(len(mean_fitness)),mean_fitness,label="Mean")
    plotting.plot(range(len(max_fitness)),max_fitness,label="Max Fitness")
    plotting.plot(range(len(min_fitness)),min_fitness,label="Min Fitness")
